...
extern void xkvm_go_log_handler(int level, cchar_t *filename, cchar_t *funcname, int line, cchar_t *message);
static inline void xkvm_cgo_setup_log_handler() {
    xkvm_set_log_handler(&xkvm_go_log_handler);
}

extern void xkvm_go_video_state_handler(xkvm_video_state_t *state);
static inline void xkvm_cgo_setup_video_state_handler() {
    xkvm_set_video_state_handler(&xkvm_go_video_state_handler);
}

extern void xkvm_go_video_handler(cuint8_t *frame, ssize_t len);
static inline void xkvm_cgo_setup_video_handler() {
    xkvm_set_video_handler(&xkvm_go_video_handler);
}

extern void xkvm_go_indev_handler(int code);
static inline void xkvm_cgo_setup_indev_handler() {
    xkvm_set_indev_handler(&xkvm_go_indev_handler);
}

extern void xkvm_go_rpc_handler(cchar_t *method, cchar_t *params);
static inline void xkvm_cgo_setup_rpc_handler() {
    xkvm_set_rpc_handler(&xkvm_go_rpc_handler);
}
*/
import "C"
...
//export xkvm_go_video_state_handler
func xkvm_go_video_state_handler(state *C.xkvm_video_state_t) {
...
//export xkvm_go_log_handler
func xkvm_go_log_handler(level C.int, filename *C.cchar_t, funcname *C.cchar_t, line C.int, message *C.cchar_t) {
...
//export xkvm_go_video_handler
func xkvm_go_video_handler(frame *C.cuint8_t, len C.ssize_t) {
	videoFrameChan <- C.GoBytes(unsafe.Pointer(frame), C.int(len))
}

//export xkvm_go_indev_handler
func xkvm_go_indev_handler(code C.int) {
	indevEventChan <- int(code)
}

//export xkvm_go_rpc_handler
func xkvm_go_rpc_handler(method *C.cchar_t, params *C.cchar_t) {
...
func setUpNativeHandlers() {
	...
	C.xkvm_cgo_setup_log_handler()
	C.xkvm_cgo_setup_video_state_handler()
	C.xkvm_cgo_setup_video_handler()
	C.xkvm_cgo_setup_indev_handler()
	C.xkvm_cgo_setup_rpc_handler()
}
...
func videoGetStreamingStatus() VideoStreamingStatus {
	...
	isStreaming := C.xkvm_video_get_streaming_status()
...
func videoLogStatus() string {
	...
	logStatus := C.xkvm_video_log_status()
...
func videoGetStreamQualityFactor() (float64, error) {
	...
	factor := C.xkvm_video_get_quality_factor()
...
func videoSetStreamQualityFactor(factor float64) error {
	...
	C.xkvm_video_set_quality_factor(C.float(factor))
	return nil
}
...
func videoGetEDID() (string, error) {
	...
	edidCStr := C.xkvm_video_get_edid_hex()
...
func videoSetEDID(edid string) error {
	...
	C.xkvm_video_set_edid(edidCStr)
	return nil
}
